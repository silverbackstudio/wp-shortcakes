...
class Thumb extends Shortcake {
    ...
    public function renderOutput($attr, $content, $shortcode_tag){
    	...
    	if( ! $term ) {
    	    ...
    	    if ( $this->image_field && function_exists('get_field') && ($image_id = get_field( $this->image_field, $this->taxonomy . '_'. $attr['term_id'] ) ) ) {
    	        $output['image'] = wp_get_attachment_image($image_id, $this->image_size);
    	    }
    	    
    	    if( $description = get_term_field( 'description', $term ) ) {
